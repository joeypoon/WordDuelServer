...
export function connectionHandler (socket) {
    ...
    socket.on(events.players.search, () => players.search(socket));
    socket.on(events.players.ready, () => players.ready(socket));
    socket.on(events.search.cancel, () => players.searchCancel(socket));
    ...
    // matches
    socket.on(events.matches.join, data => matches.onMatchJoin(socket, data));
