...
export function connectionHandler (socket) {
    ...
    socket.on(events.search.cancel, data => connectionHandlers.onPlayerSearchCancel(data));
    socket.on(events.score.submit, data => connectionHandlers.onScoreSubmit(socket, data))
