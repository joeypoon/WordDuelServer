...
export function onChallengeResponse(socket, data) {
    const { response, facebookId, opponentSocket, matchId } = data;
    if (matchId) {
    ...
    if (response) {
        Match.create({ challenge: true })
            .then(match => {
                match.joinMatch(facebookId, socket);
                io.to(opponentSocket).emit(
                    events.players.challengeResponse,
                    { response, matchId: match._id }
                );
            });
    } else {
        io.to(opponentSocket)
            .emit(events.players.challengeResponse, { response });
    }
}